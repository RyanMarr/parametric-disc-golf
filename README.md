...
- `flat_top_chamfer` (30–60°, flat-top only) — the round shoulder naturally leaves the flat plane horizontally, which is a 90° overhang printed top-down (the first layers droop right where your thumb sits). This clamps the shoulder under a straight chamfer at the given angle from horizontal (default 45° = safe on any printer) that blends back into the original curve where it's naturally steeper. Lower values keep more of the round shoulder but need a printer that handles shallow overhangs. The designer shows the resulting worst-case print overhang live and warns when a wide/low rim can't fully resolve (a small flat ring remains at the parting line).
